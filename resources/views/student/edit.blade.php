...
<form action="{{ route('student.update', $id['id']) }}" class="card mt-3 p-5" method="POST" style="max-width: 900px">
    @if ($errors->any())
    ...
    <div class="mb-3 row">
        ...
        <div class="col-sm-10">
        <input type="text" class="form-control" id="nis" name="nis" value="{{ $id['nis'] }}">
        @error('nis')
        ...
        <div class="col-sm-10">
           <select name="rombel" id="rombel" class="form-select">
                ...
                @foreach ($students as $item)
                    <option value="{{ $item['rombel'] }}" {{ old('rombel') }}>{{ $item['rombel'] }}</option>
                @endforeach
           </select>
        </div>
    </div>
    <div class="mb-3 row">
        <div class="col-sm-10">
           <select name="rayon" id="rayon" class="form-select">
                ...
                @foreach ($students as $item)
                    <option value="{{ $item['rayon'] }}" {{ old('rayon') }}>{{ $item['rayon'] }}</option>
                @endforeach
           </select>
        </div>
    </div>
